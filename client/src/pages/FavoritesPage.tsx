      ...
      <div className="space-y-6">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.35em] text-sky-600">My Favorites</p>
              <h1 className="text-3xl font-bold text-slate-900">Saved products</h1>
              <p className="mt-2 text-sm text-slate-600">Keep an eye on listings you're interested in and revisit them later.</p>
            </div>
        ...
        {loading ? (
          <div className="rounded-3xl border border-slate-200 bg-white p-8 text-center text-slate-700">Loading your favorites…</div>
        ) : error ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {favorites.map((product) => (
              <div key={product._id} className="group relative rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:shadow-xl hover:border-sky-200">
                <button
                  type="button"
