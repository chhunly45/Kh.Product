                ...
                {loading ? (
                  ...
                  <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
                    {products.map((product) => (
                      <article key={product._id || product.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        <div className="flex flex-col gap-3">
                          <div className="flex items-center gap-4">
                            <img src={product.images?.[0]?.secureUrl || product.images?.[0]?.url || ''} alt={product.title} className="h-20 w-28 object-cover rounded-lg bg-slate-100" />
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-2 text-sm font-semibold text-slate-700 ring-1 ring-slate-200 hover:bg-slate-100 transition"
                            >
                              Edit
                            </button>
                            <button
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  await (await import('../services/product.api')).deleteProduct(product._id);
                                  setProducts((current) => current.filter((p) => p._id !== product._id));
                                } catch (err) {
                                  setStatusMessage('Unable to delete product.');
                                }
                              }}
                              className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-2 text-sm font-semibold text-rose-600 ring-1 ring-rose-200 hover:bg-rose-50 transition"
                            >
                              Delete
                            </button>
                            {product.status !== 'sold' && (
                              <button
                                ...
                                onClick={async () => {
                                  try {
                                    await (await import('../services/product.api')).updateProduct(product._id, { status: 'sold' });
                                    setProducts((current) => current.map((p) => (p._id === product._id ? { ...p, status: 'sold' } : p)));
                                  } catch (err) {
                                    setStatusMessage('Could not update product status.');
                                  }
                                }}
                                className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800 transition"
                              >
                                Mark sold
                              </button>
                            )}
